...
export class DocumentProcessor {
    ...
    constructor() {
        ...
        this.chunker = new TextChunker(500); // Using a reasonable default chunk size
    ...
    async processDocument(tenantId: string, document: Buffer): Promise<ProcessingResult> {
        try {
            ...
            if (this.azureSearchClient) {
                try {
                    // Get the Azure Search index name from config or use the realestate index
                    ...
                    // Convert vectors to Azure Search documents
                    ...
                    // Get the search client for the index
                    const searchClient = this.azureSearchClient.getSearchClient(azureIndexName);

                    // Upload documents to Azure Search
                    storageResult = await searchClient.uploadDocuments(searchDocuments);
            ...
            // Fall back to Pinecone if Azure Search failed or wasn't available
            console.log(`[${new Date().toISOString()}] Using Pinecone for vector storage`);
            const indexName = "myvectordbb";
            ...
            try {
                ...
                console.log(`[${new Date().toISOString()}] Successfully stored vectors in Pinecone index: ${indexName}`);
     ...
     */
    private async verifyStorage(indexName: string, vectorId: string): Promise<void> {
        try {
            console.log(`[${new Date().toISOString()}] Verifying storage of vector: ${vectorId}`);

            // Attempt to fetch the vector to confirm it was stored
            const pineconeInstance = PineconeService.getInstance();
            const index = pineconeInstance.Index(indexName);
    ...
    async checkVectorsExist(tenantId: string, vectorIds: string[]): Promise<{ [id: string]: boolean }> {
        try {
            const pineconeInstance = PineconeService.getInstance();
            const index = pineconeInstance.Index("realestate-us-sample-index");
        ...
        try {
            ...
            if (this.azureSearchClient) {
                try {
                    ...
                    // Use vector search with the query embedding
            ...
            // Fall back to Pinecone search
            console.log(`[${new Date().toISOString()}] Searching in Pinecone`);

            const pineconeIndexName = "myvectordbb";
            const pineconeInstance = PineconeService.getInstance();
            const index = pineconeInstance.Index(pineconeIndexName);
            ...
            // Map Pinecone results to SearchResult format
